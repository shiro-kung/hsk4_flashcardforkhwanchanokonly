...
/** จากกดถัดไปบนหน้าคำจีนจนถึงการไปคำถัดไป (ms) รวมเวลาพลิก */
const ADVANCE_FROM_FRONT_TOTAL_MS = 1000;
/** เมื่อพลิกอยู่แล้ว: รอก่อนไปคำถัดไป (ms) */
const READ_BEFORE_NEXT_MS = 1000;
...
function injectScoreUI() {
  ...
  wrap.innerHTML = `
    ...
    <p class="score-hint">นับคะแนน<strong>ข้อละ 1 คะแนน</strong>เมื่อกด <strong>ถัดไป</strong> / จำได้ • <strong>ถัดไป</strong> / <kbd>→</kbd> = จำได้แล้ว: ถ้ายังเป็นหน้าคำจีน ระบบจะพลิกให้เห็นพินอิน แล้วไปคำถัดไปภายในราว 1 วินาทีหลังกด (รวมเวลาพลิก) • ถ้าพลิกอยู่แล้ว รอประมาณ 1 วินาที แล้วไปคำถัดไป • <kbd>Space</kbd> พลิกเอง • <kbd>←</kbd> ก่อนหน้า (ไม่นับคะแนน) • <kbd>K</kbd> เหมือนถัดไป • <kbd>J</kbd> ยังไม่แม่น</p>
  `;
  topbar.after(wrap);
...
/** ถัดไป / จำได้: นับคะแนน ถ้ายังไม่พลิกจะพลิกให้เห็นคำตอบก่อน แล้วไปคำถัดไป (หน้าคำจีน = รวม ~1 วิ หลังกด) */
function advanceAsKnown() {
  ...
  if (!flipped) {
    flipped = true;
    renderCard();
    // รวมเวลาหลังกดถึงไปคำถัดไป = ADVANCE_FROM_FRONT_TOTAL_MS (รวมพลิก); ต้องไม่เร็วกว่าพลิกจบมิฉะนั้นจะเห็นคำแปลผิดคำ
    const flipDoneMs = window.matchMedia("(prefers-reduced-motion: reduce)").matches
      ? 80
      : 620;
    advanceTimer = setTimeout(goNext, Math.max(ADVANCE_FROM_FRONT_TOTAL_MS, flipDoneMs + 50));
  } else {
    advanceTimer = setTimeout(goNext, READ_BEFORE_NEXT_MS);
  }
}
